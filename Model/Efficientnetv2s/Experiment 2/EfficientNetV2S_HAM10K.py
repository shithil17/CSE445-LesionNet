...
class EfficientNetFinetuneTrainer:
    ...
    def fit_temperature(self, model, val_loader, n_augments=4):
        ...
        if ece_at_T < ece_at_1:
            self._temperature_fallback_single = False
            print(f"    Fitted T={fitted_T:.4f} improved val ECE "
                  f"({ece_at_1:.4f} -> {ece_at_T:.4f}) — accepted.")
            return fitted_T
        self._temperature_fallback_single = True
        print(f"    Fitted T={fitted_T:.4f} gave worse val ECE ({ece_at_T:.4f}) "
              f"than T=1.0 ({ece_at_1:.4f}) — falling back to T=1.0.")
        return 1.0
    ...
    def fit_temperature_on_probs(self, val_probs, val_targets):
        ...
        if ece_at_T < ece_at_1:
            self._temperature_fallback_ens = False
            print(f"    Ensemble fitted T={fitted_T:.4f} improved val ECE "
                  f"({ece_at_1:.4f} -> {ece_at_T:.4f}) — accepted.")
            return fitted_T
        self._temperature_fallback_ens = True
        print(f"    Ensemble fitted T={fitted_T:.4f} gave worse val ECE "
              f"({ece_at_T:.4f}) than T=1.0 ({ece_at_1:.4f}) — "
    ...
    def _final_metrics_report(self, targets, preds, probs, acc, best_epoch, ece,
        ...
        if temperature is not None:
            ...
            if temperature_fallback:
                print("    Fitted T           : 1.0 (fit rejected — see fallback "
                      "note above)")
            else:
                ...
                if val_probs_ens is None:
                    ...
                    if use_ensemble_final:
                        print("    Ensemble temperature (fitted on val probs): "
                              + ("1.0 (fit rejected — see fallback note above)"
