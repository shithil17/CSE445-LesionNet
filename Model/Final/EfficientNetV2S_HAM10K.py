...
Current recipe:
  - ImageNet normalization from the active torchvision weights; aspect-preserving
    resize/crop for train and eval.
  - Discriminative LRs: head 1e-3, early-unfreeze blocks = head * 0.1,
    stage-2 blocks = head * 0.05.
  - ReduceLROnPlateau on validation Macro-F1 (mode='max', factor=0.5,
    patience=5, min_lr=1e-7).
  - Gradual unfreezing: head epochs 1-5, early blocks at epoch 6, then
...
MLP fused with the selected EfficientNet features.
...
class EfficientNetFinetuneTrainer:
    """Selected EfficientNet backbone with ImageNet preprocessing, discriminative
    ...
    Key arguments:
      ...
      - head epochs 1-5, early blocks at epoch 6, stage-2 blocks plateau-gated;
    ...
    def _set_trainable(self, model, epoch_idx):
        ...
        Early-unfreeze blocks: fixed unfreeze at 1-based epoch 6 (0-based idx 5).
